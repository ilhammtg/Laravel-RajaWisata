    ...
    <div class="w-full max-w-md">
      <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
        <div class="bg-hero-aceh px-8 pt-10 pb-8 text-center">
          ...
          <h1 class="text-white text-xl font-bold mt-4">Buat Akun Baru</h1>
          <p class="text-pala-light text-sm mt-1">Bergabung dan nikmati wisata Aceh Selatan</p>
        </div>

        <div class="px-8 py-8">
          <div v-if="Object.keys(form.errors).length > 0" class="mb-5 flex items-start gap-3 bg-red-50 border border-red-200 text-red-700 rounded-lg px-4 py-3 text-sm">
            ...
            <span>{{ form.errors[Object.keys(form.errors)[0]] }}</span>
          </div>
          ...
          <p class="text-center text-sm text-gray-500 mt-6">
            Sudah punya akun?
            <Link href="/login" class="text-pala font-semibold hover:text-pala-dark ml-1">Masuk</Link>
          </p>
        </div>
      </div>
...
// Menggunakan useForm untuk handle registrasi otomatis
const form = useForm({
  name: '',
  email: '',
  password: '',
